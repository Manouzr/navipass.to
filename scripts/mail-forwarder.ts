...
async function main() {
  ...
  const { mailGwGetToken, mailGwGetMessages, mailGwGetMessage, mailGwMarkRead } = await import('../src/lib/mailgw')
  ...
  async function poll() {
    ...
    for (const order of orders) {
      try {
        ...
        const cutoff = order.mailGwLastCheckedAt ?? new Date(0)
        ...
        for (const msg of newMessages) {
          ...
          const htmlBody = full.html[0] ?? `<p>${full.text ?? msg.intro}</p>`
